...
export function wealthAllocationSegmentDescription(
  ...
  switch (key) {
    case "heirs":
      if (path === "stay") {
        return `Traditional IRA balance projected at the end of the plan, reduced by an assumed ${data.assumedHeirTaxRatePct}% beneficiary ordinary income tax at death. This is illustrative only. Not estate tax and not tax advice.`;
      }
    ...
    case "income":
      return "After-tax retirement income you retain during the modeled lifetime: withdrawals and spendable cash flow from the account pool after illustrative federal taxes on those distributions.";
    case "taxes":
...
export function bracketZoneDescription(
  ...
  switch (zone) {
    ...
    case "ceiling":
      return `Income cap at ${formatRothMoneyFull(data.grossIncomeCeiling)} gross income. Conversions use available room each year without pushing ordinary income into the next bracket${nextBracket ? ` (${nextBracket}%)` : ""}.`;
    case "avoid":
      return "Income above your selected bracket would cross into a higher marginal tax rate. The conversion plan avoids sizing conversions so large that you push income above your target bracket.";
    default:
      return "";
  }
}
